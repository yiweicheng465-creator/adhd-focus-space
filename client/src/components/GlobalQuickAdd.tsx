...
export function GlobalQuickAdd({ onAddTask }: GlobalQuickAddProps) {
  ...
  const { data: dbChips } = trpc.quickReplies.list.useQuery(undefined, {
    staleTime: 60_000,
  });
  const createChip = trpc.quickReplies.create.useMutation({
    onSuccess: () => utils.quickReplies.list.invalidate(),
  });
  const deleteChip = trpc.quickReplies.delete.useMutation({
    onSuccess: () => utils.quickReplies.list.invalidate(),
  });

  // Use DB chips if any exist, otherwise fall back to defaults
  const chips: string[] =
    dbChips && dbChips.length > 0
      ? dbChips.map((c) => c.text)
      : DEFAULT_CHIPS;
  ...
  const handleAddChip = () => {
    ...
    // If no DB chips yet, seed defaults first, then add new one
    const seedAndAdd = async () => {
      const existing = dbChips ?? [];
      if (existing.length === 0) {
        for (let i = 0; i < DEFAULT_CHIPS.length; i++) {
          await createChip.mutateAsync({ id: nanoid(), text: DEFAULT_CHIPS[i], sortOrder: i });
        }
      }
      await createChip.mutateAsync({ id: nanoid(), text: trimmed, sortOrder: (dbChips?.length ?? DEFAULT_CHIPS.length) });
    };
    seedAndAdd().catch(() => toast.error("Failed to save chip"));
    setNewChip("");
  };

  // ── Delete chip ───────────────────────────────────────────────────────────
  const handleDeleteChip = (chipText: string) => {
    const chip = dbChips?.find((c) => c.text === chipText);
    if (!chip) {
      // Chip is from defaults — seed all defaults first, then delete the target
      const seedAndDelete = async () => {
        const ids: Record<string, string> = {};
        for (let i = 0; i < DEFAULT_CHIPS.length; i++) {
          const id = nanoid();
          ids[DEFAULT_CHIPS[i]] = id;
          await createChip.mutateAsync({ id, text: DEFAULT_CHIPS[i], sortOrder: i });
        }
        await deleteChip.mutateAsync({ id: ids[chipText] });
      };
      seedAndDelete().catch(() => toast.error("Failed to delete chip"));
      return;
    }
    deleteChip.mutate({ id: chip.id });
  };
